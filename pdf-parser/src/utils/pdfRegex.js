/**
 * PDF_REGEX provides a centralised collection of Regular Expressions
 * for parsing PDF structure, images, CMap, and text streams.
 *
 * Ported from pdf/utils/pdfRegex.js — CommonJS → ESM named export.
 * All regex patterns are unchanged; they operate on strings in both runtimes.
 */
export const PDF_REGEX = {
    common: {
        whitespace:  /\s+/,
        numeric:     /-?\d+(\.\d+)?/,
        lineBreaks:  /\r?\n/
    ...
    text: {
        ...
        // Content Stream — individual parts inside a TJ array (hex strings, literal strings, or kern numbers)
        tjParts:             /<[^>]+>|\([^)]+\)|-?\d+(?:\.\d+)?/g,
